...
void drawBitmap(const unsigned long* image, int scrX, int scrY, int w, int h)
{
	...
	for (j = 0 + scrY; j < h + scrY; j++)
	{
		if((j < vmode->xfbHeight)&&(j < vmode->efbHeight)&&(j < vmode->viHeight)) {
			for (i = 0; i < w/2; i++)
			{
				xfb[whichfb][(vmode->fbWidth/2) * j + i + scrX] = image[current];
				current++;
			}
		}
	}
...
void drawBitmapTransparency(const unsigned long* image, int scrX, int scrY, int w, int h, int transparency)
{
	int i, j;
	int current = 0;
...
void DrawMessageBox(int type, char *message) {
  ...
  WriteFont(x2-GetTextSizeInPixels(typeToStr(type))-10,y2-34,typeToStr(type));
}
